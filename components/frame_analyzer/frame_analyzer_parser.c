 ...
 * @brief Implements 802.11 / EAPoL frame parsing.
 */
...
/**
 * @brief Walks key data buffer looking for PMKID KDEs, building a linked list of matches.
 */
static pmkid_item_t *parse_pmkid_from_key_data(uint8_t *key_data, const uint16_t length) {
    ...
    do {
        ...
        if (ntohl(key_data_field->oui) != KEY_DATA_OUI_IEEE80211) {
            ESP_LOGD(TAG, "Wrong OUI %lx (expected %x)", (unsigned long) key_data_field->oui, KEY_DATA_OUI_IEEE80211);
            continue;
        }
        ...
        if (pmkid_item == NULL) {
            ESP_LOGE(TAG, "Failed to allocate pmkid_item_t");
            break;
        }
